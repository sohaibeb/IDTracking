...
class IDManager {
    ...
    setupModalEvents() {
        // Botón para copiar en el modal
        document.getElementById('modalCopyBtn').addEventListener('click', () => {
            if (currentModalText) {
                copyToClipboard(currentModalText, document.getElementById('modalCopyBtn'));
            }
        });
    ...
    setupAutoSave() {
        ...
        document.getElementById('exportFullCsv').addEventListener('click', (e) => {
            e.preventDefault();
            this.exportToCsv('full');
        });
        
        document.getElementById('exportDecoderCsv').addEventListener('click', (e) => {
            e.preventDefault();
            this.exportToCsv('decoder');
        });
    }
            ...
            row.innerHTML = `
                ...
                <td>
                    <div class="d-flex justify-content-between align-items-center">
                        ...
                        <div class="action-buttons">
                            <span class="view-btn ms-2" data-text="${item.base64}" title="Ver texto completo">
                                <i class="bi bi-eye"></i>
                            </span>
                            <span class="copy-btn" data-text="${item.base64}" title="Copiar Base64">
                ...
                <td>
                    <div class="d-flex justify-content-between align-items-center">
                        ...
                        <div class="action-buttons">
                            <span class="view-btn ms-2" data-text="${item.rotated}" title="Ver texto completo">
                                <i class="bi bi-eye"></i>
                            </span>
                            <span class="copy-btn" data-text="${item.rotated}" title="Copiar ID final">
                                <i class="bi bi-clipboard"></i>
                            </span>
                        </div>
                    </div>
        ...
        this.decodedIDs.forEach((item, index) => {
            ...
            row.innerHTML = `
                <td class="text-truncate" style="max-width: 200px;">
                    <div class="d-flex justify-content-between align-items-center">
                        <span>${item.encodedID}</span>
                        <div class="action-buttons">
                            <span class="view-btn ms-2" data-text="${item.encodedID}" title="Ver texto completo">
                                <i class="bi bi-eye"></i>
                            </span>
                            <span class="copy-btn" data-text="${item.encodedID}" title="Copiar ID">
                                <i class="bi bi-clipboard"></i>
                            </span>
                        </div>
                    </div>
    ...
    exportToCsv(type = 'full') {
        ...
        if (type === 'full') {
            ...
            const headers = ['Año', 'Versión', 'Empresa', 'Última Letra', 'Posición', 'Base64', 'ID Final', 'Rotación', 'Fecha'];
            csvContent = headers.join(',') + '\n';
            
            this.generatedIDs.forEach(item => {
                const row = [
                    `"${item.year}"`,
                    `"${item.version}"`,
                    `"${item.company}"`,
                    `"${item.lastLetter}"`,
                    `"${item.lastLetterPosition}"`,
                    `"${item.base64}"`,
                    `"${item.rotated}"`,
                    `"${item.rotation}"`,
                    `"${item.timestamp}"`
            ...
            filename = `ids_completo_${new Date().toISOString().slice(0, 10)}.csv`;
        } else if (type === 'decoder') {
            // Exportar para decodificador
            const headers = ['ID Final', 'Clave'];
            csvContent = headers.join(',') + '\n';
            ...
            filename = `ids_para_decodificar_${new Date().toISOString().slice(0, 10)}.csv`;
        }
        ...
        showToast(`CSV exportado: ${filename}`, 'success');
    }
    
    // Importar CSV para decodificación
    importCsvForDecoding() {
        ...
        reader.onload = (e) => {
            try {
                ...
                if (lines.length < 2) {
                    showToast('El archivo CSV está vacío o no tiene el formato correcto', 'warning');
                    return;
                }
                ...
                const hasIdFinal = headers.some(h => h.toLowerCase().includes('id final') || h.toLowerCase().includes('id_final'));
                const hasClave = headers.some(h => h.toLowerCase().includes('clave'));
                
                if (!hasIdFinal || !hasClave) {
                    showToast('El archivo CSV debe contener las columnas "ID Final" y "Clave"', 'warning');
                    return;
                }
                
                // Determinar índices de columnas
                const idIndex = headers.findIndex(h => h.toLowerCase().includes('id final') || h.toLowerCase().includes('id_final'));
                const claveIndex = headers.findIndex(h => h.toLowerCase().includes('clave'));
                ...
                for (let i = 1; i < lines.length; i++) {
                    ...
                    if (cells.length >= Math.max(idIndex, claveIndex) + 1) {
                        const encodedId = cells[idIndex];
                        const keyLetter = cells[claveIndex];
                        
                        if (encodedId && keyLetter) {
                            const result = this.decodeID(encodedId, keyLetter);
                            this.addDecodedID(encodedId, keyLetter, result);
                            importedCount++;
                        } else {
                            errorCount++;
                        }
                ...
                // Mostrar resultado
                showToast(`Importación completada: ${importedCount} IDs procesados, ${errorCount} errores`, 'success');
...
document.addEventListener('click', function(e) {
    // Botones de ver (ojo) en las tablas
    if (e.target.closest('.view-btn')) {
        const viewBtn = e.target.closest('.view-btn');
        const textToShow = viewBtn.getAttribute('data-text');
        if (textToShow) {
            showTextInModal(textToShow);
        }
    }
    
    // Botones de copiar en las tablas
    ...
    // Botones de eliminar en las tablas
